...
from ZEEMUSIC import app, Userbot
...
from ZEEMUSIC.utils.admin_filters import admin_filter
from config import BANNED_USERS

@app.on_message(filters.command(["vcinfo"], ["/", "!"]))
async def strcall(client, message):
    assistant = await group_assistant(ZEE, message.chat.id)
    try:
        await assistant.join_group_call(message.chat.id, AudioPiped("./ZEEMUSIC/assets/call.mp3"), stream_type=StreamType().pulse_stream)
...
async def set_volume(client, message: Message):
    ...
    try:
        await ZEE.change_volume(chat_id, volume_level)
